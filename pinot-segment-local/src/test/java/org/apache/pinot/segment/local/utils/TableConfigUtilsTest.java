...
public class TableConfigUtilsTest {
  ...
  public void validateIngestionConfig() {
    ...
    ingestionConfig.setFilterConfig(null);
    ingestionConfig.setTransformConfigs(Collections.singletonList(new TransformConfig(null, "reverse(anotherCol)", null,
        null)));
    try {
      TableConfigUtils.validate(tableConfig, schema);
    ...
    // invalid function
    ingestionConfig.setTransformConfigs(Collections.singletonList(new TransformConfig("myCol", "Groovy(badExpr)", null,
        null)));
    try {
      TableConfigUtils.validate(tableConfig, schema);
      Assert.fail("Should fail for invalid transform function in transform config");
    ...
    ingestionConfig.setTransformConfigs(Collections.singletonList(new TransformConfig("myCol", "reverse(myCol)", null,
        null)));
    try {
      TableConfigUtils.validate(tableConfig, schema);
    ...
    ingestionConfig.setTransformConfigs(
        Collections.singletonList(new TransformConfig("myCol", "Groovy({x + y + myCol}, x, myCol, y)", null, null)));
    try {
      TableConfigUtils.validate(tableConfig, schema);
      Assert.fail("Should fail due to use of myCol as arguments and columnName");
    } catch (IllegalStateException e) {
    ...
    ingestionConfig.setTransformConfigs(
        Arrays.asList(new TransformConfig("myCol", "reverse(x)", null, null), new TransformConfig("myCol", "lower(y)",
            null, null)));
    try {
      TableConfigUtils.validate(tableConfig, schema);
    ...
    ingestionConfig.setTransformConfigs(Arrays.asList(new TransformConfig("transformedCol", "reverse(x)", null, null),
