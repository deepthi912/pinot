...
public class RecordTransformerRegistry {
  ...
  public static void validateEnrichmentConfig(TransformConfig transformConfig,
    ...
    RECORD_ENRICHER_FACTORY_MAP.get(transformConfig.getEnricherType())
        .validateEnrichmentConfig(transformConfig.getProperties(), config);
  }
  ...
  public static RecordTransformer createRecordEnricher(TransformConfig enrichmentConfig)
    ...
    return RECORD_ENRICHER_FACTORY_MAP.get(enrichmentConfig.getEnricherType())
        .createTransformer(enrichmentConfig.getProperties());
  }

  static {
    for (RecordTransformerFactory recordTransformerFactory : ServiceLoader.load(RecordTransformerFactory.class)) {
      LOGGER.info("Registered record enricher factory type: {}", recordTransformerFactory.getEnricherType());
      RECORD_ENRICHER_FACTORY_MAP.put(recordTransformerFactory.getEnricherType(), recordTransformerFactory);
    }
  }
}
